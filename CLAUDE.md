...
- Custom bindings: `prefix+up/down` and `prefix+shift+1..9` switch workspaces, `prefix+,`/`prefix+.` cycle agents, `prefix+o` last pane, `prefix+a` new Claude Code pane, `prefix+shift+u` focus next blocked/done agent (via `herdr/bin/herdr-focus-attention`, symlinked to `~/.local/bin/`)
